...
function et(e, a) {
  ...
  const s = r.get("type"), o = r.get("access_token");
  if (s === "invite" || s === "recovery") return o ? { kind: s, token: o } : { kind: "error" };
  if (s === "signup") return o ? { kind: "signup" } : { kind: "error" };
  const c = i.get("recovery_token");
  return c ? { kind: "recovery", token: c } : { kind: "none" };
}
const t = (e) => document.querySelector(e), M = (e) => [...document.querySelectorAll(e)], l = { mode: "login", user: null, profile: null, books: [], offers: [], preferences: null, dashboard: null, gamification: null, alerts: [], notifications: [], shelf: "reading", chatHistory: [], chatSessionId: null, compareIds: /* @__PURE__ */ new Set(), lastRecommendations: [], lastDiscoveryQuery: "", recommendationId: null, recommendationVariant: null, catalog: { offset: 0, limit: 12, total: 0, timer: null, coverage: null, items: [] } }, tt = ["İçe dönük", "Analitik", "Stratejik", "Melankolik", "Maceracı", "İdealist", "Meraklı", "Bağımsız", "Duygusal"];
function n(e, a = "", i = "") {
...
function f(e, a = !1) {
  const i = t("#toast");
  i.textContent = e, i.className = `toast show${a ? " error" : ""}`, clearTimeout(Ee), Ee = setTimeout(() => i.className = "toast", 3200);
}
function ge(e, a = {}) {
...
function at() {
  ...
  }, t("#sidebar-backdrop").onclick = he, H.addEventListener("change", () => {
    document.body.classList.remove("sidebar-open"), document.body.classList.toggle("sidebar-collapsed", !H.matches && localStorage.getItem("mihenk-sidebar-collapsed") === "true"), j(), we();
  }), document.addEventListener("keydown", (e) => {
    e.key === "Escape" && document.body.classList.contains("sidebar-open") && he();
  }), j();
}
function Pe(e) {
  const a = n("span", "", `cover-fallback cover-theme-${nt(e.genre)}`), i = n("span", (e.genre || "Edebiyat").split("/")[0].trim(), "cover-genre"), r = n("strong", e.title), s = n("small", e.author || "");
...
function F(e, a = "cover-wrap") {
  ...
  if (e.cover_url) {
    const r = new Image();
    r.src = e.cover_url, r.alt = `${e.title} kapağı`, r.loading = "lazy", r.referrerPolicy = "no-referrer", r.onerror = () => i.replaceChildren(Pe(e)), i.append(r);
  } else i.append(Pe(e));
  return i;
}
function se(e) {
...
t("#auth-switch").onclick = () => se(l.mode === "login" ? "register" : "login");
function _e() {
  se("login");
  const e = t("#login-dialog");
  e.open || e.showModal(), setTimeout(() => t("#auth-email").focus(), 0);
}
M("[data-open-login]").forEach((e) => e.onclick = _e);
t("#demo-open").onclick = () => {
...
const V = et(location.search, location.hash);
let ue = V.token || "";
const W = V.kind === "invite";
V.kind !== "none" && (history.replaceState({}, "", location.pathname), V.kind === "error" ? setTimeout(() => {
  t("#recovery-status").textContent = "Bağlantı geçersiz veya süresi dolmuş. Yeni bir sıfırlama bağlantısı isteyin. Davet için yöneticiden yeni bağlantı isteyebilirsiniz.", t("#recovery-dialog").showModal();
}, 0) : V.kind === "signup" ? setTimeout(() => {
  _e(), t("#auth-error").textContent = "E-posta adresin doğrulandı. Parolanla giriş yapabilirsin.";
}, 0) : (t("#password-reset-title").textContent = W ? "Mihenk’e hoş geldin" : "Hesabını güvene al", t("#password-reset-description").textContent = W ? "Davetini tamamlamak için bir parola belirle. Ardından e-posta adresinle giriş yapabilirsin." : "Hesabın için yeni bir parola belirle.", t("#password-reset-submit").textContent = W ? "Parola oluştur" : "Parolayı güncelle", setTimeout(() => t("#password-reset-dialog").showModal(), 0)));
t("#password-reset-form").onsubmit = async (e) => {
  e.preventDefault();
  const a = t("#new-password").value, i = t("#new-password-confirm").value, r = t("#password-reset-submit");
  if (a !== i) {
    t("#password-reset-status").textContent = "Parolalar eşleşmiyor.";
    return;
  }
  if (!ue) {
    t("#password-reset-status").textContent = "Yeni bir davet ya da parola sıfırlama bağlantısı isteyin.";
    return;
  }
  r.disabled = !0, r.textContent = "Kaydediliyor…";
  try {
    await g("/auth/password/reset", { method: "POST", body: JSON.stringify({ recovery_token: ue, new_password: a }) }), ue = "", t("#password-reset-form").reset(), t("#password-reset-dialog").close(), _e(), t("#auth-error").textContent = W ? "Parolan oluşturuldu. Davet edilen e-posta adresin ve yeni parolanla giriş yapabilirsin." : "Parolan güncellendi. Yeni parolanla giriş yapabilirsin.";
  } catch (s) {
    t("#password-reset-status").textContent = s.message;
  } finally {
    r.disabled = !1, r.textContent = W ? "Parola oluştur" : "Parolayı güncelle";
  }
};
t("#auth-form").onsubmit = async (e) => {
  ...
  try {
    if (l.user = await g(`/auth/${l.mode}`, { method: "POST", body: JSON.stringify(i) }), l.user.email_confirmation_required) {
      se("login"), t("#auth-error").textContent = "E-posta adresine gönderilen bağlantıyla hesabını doğrula.";
      return;
    }
    await Be();
  } catch (r) {
...
t("#beta-feedback-form").onsubmit = async (e) => {
  ...
  try {
    await g("/me/beta-feedback", { method: "POST", body: JSON.stringify({ category: t("#beta-feedback-category").value, rating: s === "" ? null : Number(s), message: t("#beta-feedback-message").value.trim(), context: { view: fe || "settings", viewport: `${window.innerWidth}x${window.innerHeight}` } }) }), a.reset(), r.textContent = "Teşekkürler, geri bildirimin kaydedildi.", f("Geri bildirimin ürün ekibine ulaştı.");
  } catch (o) {
...
function it() {
  se("login"), K(!1), t("#login-dialog").open && t("#login-dialog").close(), t("#app").classList.add("hidden"), t("#chat-toggle").classList.add("hidden"), t("#auth-screen").classList.remove("hidden");
}
async function Be(e = !1) {
  ...
  t("#login-dialog").open && t("#login-dialog").close(), t("#auth-screen").classList.add("hidden"), t("#app").classList.remove("hidden"), t("#chat-toggle").classList.remove("hidden"), t("#admin-nav").classList.toggle("hidden", !["editor", "admin"].includes(l.user.app_role)), t("#admin-invitation-panel").classList.toggle("hidden", l.user.app_role !== "admin"), rt(), X(), oe(), de(), ke(), Kt(), qt(), st(), z(!0).catch(() => {
  }), ge("session_started", { source: "web" });
  ...
  a[0].status === "fulfilled" && (l.books = a[0].value), a[1].status === "fulfilled" && (l.offers = a[1].value), a[2].status === "fulfilled" && (l.dashboard = a[2].value), a[3].status === "fulfilled" && (l.alerts = a[3].value), X(), oe(), Fe(), ke(), At(), a[4].status === "fulfilled" && !a[4].value.onboarding_completed && (G("growth"), ge("onboarding_started", { source: "first_session" }), f("Önce birkaç tercihini seç; önerilerin sana göre şekillensin.")), window.dispatchEvent(new CustomEvent("pkm-refresh"));
}
function rt() {
  var a, i;
  const e = new Set(((a = l.preferences) == null ? void 0 : a.selected_traits) || []);
  M("#traits .tag").forEach((r) => r.classList.toggle("active", e.has(r.textContent))), (i = l.preferences) != null && i.personality_text && (t("#character").value = l.preferences.personality_text), Z();
}
function X() {
  const e = l.user.display_name || "Okur", a = l.user.email || "", i = Ke(e), r = l.gamification;
  ...
  t("#notification-count").textContent = s, t("#notification-count").classList.toggle("hidden", !s), we(), ot();
}
function we() {
  var i, r;
  if (!t("#settings-view")) return;
  const e = ((i = l.user) == null ? void 0 : i.display_name) || "Okur", a = ((r = l.user) == null ? void 0 : r.email) || "";
  t("#settings-name").textContent = e, t("#settings-email").textContent = a, t("#settings-avatar").textContent = Ke(e), t("#settings-compact").checked = localStorage.getItem("mihenk-sidebar-collapsed") === "true", t("#settings-reduced-motion").checked = document.body.classList.contains("reduce-motion");
}
function ot() {
  var i, r;
  ...
  t("#home-summary").replaceChildren(...a.map(([s, o, c]) => {
    ...
    return p.type = "button", p.title = `${o} bölümünü aç`, p.append(n("b", s), n("span", o)), p.onclick = () => G(c), p;
  })), He();
}
function Me(e, a) {
  const i = n("button", e, "bento-link");
  return i.type = "button", i.onclick = () => G(a), i;
}
function He() {
  var h;
  ...
  o.append(Me(r ? "Okumaya devam et" : "Kataloğu keşfet", r ? "library" : "catalog")), s && e.append(F(s, "bento-cover")), e.append(o);
  ...
  p.append(c ? re(c, !0) : n("span", "🏆", "bento-trophy"));
  const u = n("div", "", "bento-copy");
  u.append(n("p", c ? "SIRADAKİ BAŞARIM" : "TÜMÜ TAMAM", "eyebrow"), n("h3", c ? c.name : "Edebiyat ustasısın"), n("p", c ? `${Math.max(0, c.goal - c.progress)} adım sonra +${c.xp_reward} XP kazanacaksın.` : "Tüm temel rozetleri kazandın.", "muted"), Me("Rozetlerini gör", "insights")), a.append(p, u);
...
const pe = ["Melankolik bir kış akşamı için distopik bir bilim kurgu romanı arıyorum…", "Kısa, atmosferik ve şaşırtıcı sonlu bir polisiye istiyorum…", "İçe dönük bir karakterin umut veren dönüşüm hikâyesini arıyorum…", "Savaş temalı olmayan, kolay okunan bir tarihî roman öner…"];
let Ae = null, ie = 0;
function st() {
  const e = t("#character");
  clearInterval(Ae), e.value || (e.placeholder = pe[ie]), Ae = setInterval(() => {
    document.activeElement === e || e.value || (ie = (ie + 1) % pe.length, e.classList.add("placeholder-shift"), setTimeout(() => {
      e.placeholder = pe[ie], e.classList.remove("placeholder-shift");
    }, 180));
  }, 4400), Z();
}
function Z() {
  const e = M("#traits .tag.active").map((s) => s.textContent), a = t("#character").value.trim(), i = t("#search-signal-summary");
  if (!i) return;
  const r = [];
...
document.addEventListener("click", (e) => {
  ...
  a && (G(a.dataset.view), H.matches && he());
});
t("#home-brand").onclick = () => G("discover");
let fe = "";
function G(e) {
  M(".view").forEach((i) => i.classList.toggle("hidden", i.id !== `${e}-view`)), M(".nav-button").forEach((i) => {
  ...
  document.title = `${a[e] || "Mihenk"} · Mihenk`, e === "quality" && je(), e === "community" && kt(), e === "settings" && we(), e === "reading-mode" && window.dispatchEvent(new CustomEvent("pkm-refresh")), e === "growth" && window.dispatchEvent(new CustomEvent("growth-refresh")), e !== fe && (fe = e, ge("view_opened", { view: e })), I(), window.scrollTo({ top: 0, behavior: document.body.classList.contains("reduce-motion") ? "auto" : "smooth" });
}
M("#quick-prompts button").forEach((e) => e.onclick = () => {
  const a = t("#character");
  a.value = e.dataset.prompt, a.focus(), a.setSelectionRange(a.value.length, a.value.length), Z();
});
tt.forEach((e) => {
  var i;
  ...
  a.type = "button", a.classList.toggle("active", (((i = l.preferences) == null ? void 0 : i.selected_traits) || []).includes(e)), a.onclick = () => {
    ...
    a.classList.contains("active") && !t("#character").value.toLocaleLowerCase("tr").includes(r) && (t("#character").value = t("#character").value.trim() ? `${t("#character").value.trim()}, ${r}` : r), Z();
  }, t("#traits").append(a);
});
t("#character").addEventListener("input", Z);
t("#recommend").onclick = async () => {
  ...
  const a = t("#recommend");
  a.disabled = !0, t("#recommend-status").textContent = "Katalog ve Gemma 4 birlikte düşünüyor…", Le("Arama sonuçlarını inceliyorum…", !0);
  try {
    await lt(e);
    const i = await g("/me/recommendations", { method: "POST", body: JSON.stringify({ character_description: e, limit: 6 }) });
    l.lastDiscoveryQuery = e, l.lastRecommendations = [...i.recommended_books, ...i.ai_discoveries || []], l.recommendationId = i.recommendation_id, l.recommendationVariant = i.experiment_variant, t("#recommendation-summary").textContent = i.character_analysis_summary, t("#recommendations").replaceChildren(...i.recommended_books.map((r, s) => dt(r, s + 1))), pt(i.ai_discoveries || []), t("#recommendations-panel").classList.remove("hidden"), I(), t("#recommendations-panel").scrollIntoView({ behavior: "smooth", block: "start" });
  } catch (i) {
    I(), f(i.message, !0);
  } finally {
...
async function lt(e) {
  l.preferences = await g("/me/preferences", { method: "PUT", body: JSON.stringify(qe({ personality_text: e, selected_traits: M("#traits .active").map((a) => a.textContent) })) }), He();
}
function J(e, a) {
...
function dt(e, a) {
  ...
  r.dataset.recommendationPosition = String(a), r.addEventListener("click", () => {
    r.dataset.clickTracked || (r.dataset.clickTracked = "1", Q("click", i, a));
  }), c.append(n("h3", e.book_title), n("p", `${e.author} · ${e.genre}`, "book-meta")), o.append(c, ct(e.match_score));
  ...
  if (i.id) {
    ...
    [["✓ Tam isabet", "great_match"], ["× Bana göre değil", "not_for_me"], ["Biliyorum", "already_know"], ["+ Benzerleri", "more_like_this"]].forEach(([y, h]) => {
      const b = n("button", y);
      b.type = "button", b.onclick = () => ut(i, h, b, a), m.append(b);
    }), s.append(m);
  }
  return r.append(F(i), s), r;
}
async function ut(e, a, i, r) {
  try {
    await g("/me/recommendation-feedback", { method: "PUT", body: JSON.stringify({ book_id: e.id, feedback_type: a, query_text: l.lastDiscoveryQuery || null }) }), await Q(["great_match", "more_like_this"].includes(a) ? "like" : "dislike", e, r, { feedback_type: a }), i.classList.add("selected"), i.disabled = !0, f("Geri bildirimin sonraki önerilere eklendi.");
  } catch (s) {
    f(s.message, !0);
  }
}
function pt(e) {
  t("#ai-discoveries-panel").classList.toggle("hidden", !e.length), t("#ai-discoveries").replaceChildren(...e.map(mt));
}
function mt(e) {
  ...
  return s.type = "button", s.onclick = () => ce({ title: e.book_title, author: e.author, genre: e.genre, shelf: "to_read" }), r.append(s), a.append(i, r), a;
}
function gt(e) {
  return l.offers.filter((a) => a.book_id === e).sort((a, i) => a.price_minor - i.price_minor);
}
function ht(e) {
  const a = n("div", "", "offer-list"), i = gt(e.id);
...
function xe(e) {
  const a = n("div", "", "book-actions"), i = le(e) || e, r = { to_read: "Okuyacağım", reading: "Okuyorum", read: "Okudum" }, s = n("details", "", "shelf-picker"), o = n("summary", i != null && i.shelf ? `✓ ${r[i.shelf]}` : "+ Kitaplığa ekle"), c = n("div", "", "shelf-menu");
  o.setAttribute("aria-label", i != null && i.shelf ? `Raf: ${r[i.shelf]}. Değiştir` : `${e.title} kitabını kitaplığa ekle`), s.ontoggle = () => {
    s.open && M(".shelf-picker[open]").filter((m) => m !== s).forEach((m) => m.open = !1);
    ...
    h.type = "button", h.onclick = () => {
      s.open = !1, y === "reading" ? xt(e) : Oe(e, y);
    }, c.append(h);
  }), s.append(o, c), a.append(s);
  const p = n("div", "", "compact-actions"), u = !!(i != null && i.is_favorite), d = n("button", u ? "♥" : "♡", "compact-action");
  if (d.type = "button", d.title = u ? "Favorilerden çıkar" : "Favorilere ekle", d.setAttribute("aria-label", d.title), d.setAttribute("aria-pressed", String(u)), d.onclick = () => Oe(e, (i == null ? void 0 : i.shelf) || "to_read", !u), p.append(d), e.is_custom) {
    const m = n("button", "✎", "compact-action");
    m.type = "button", m.title = "Kitabı düzenle", m.setAttribute("aria-label", m.title), m.onclick = () => ce(e);
    const y = n("button", "×", "compact-action danger");
    y.type = "button", y.title = "Kitabı sil", y.setAttribute("aria-label", y.title), y.onclick = () => $t(e), p.append(m, y);
  } else {
    const m = n("button", "♢", "compact-action");
    m.type = "button", m.title = "Fiyat alarmı kur", m.setAttribute("aria-label", m.title), m.onclick = () => Pt(e), p.append(m);
  }
  return a.append(p), a;
}
function le(e) {
...
async function Oe(e, a, i) {
  ...
  try {
    if (e.is_custom)
      await g(`/me/custom-books/${encodeURIComponent(e.id)}`, { method: "PUT", body: JSON.stringify(Ge(r, { shelf: a, is_favorite: i ?? r.is_favorite ?? !1, current_page: a === "read" ? r.total_pages || r.current_page || 0 : r.current_page || 0 })) });
    else {
      ...
      s && (await Q("library_add", e, s, { shelf: a }), a === "reading" && await Q("reading_start", e, s), a === "read" && await Q("reading_finish", e, s));
    }
...
function ft(e) {
  ...
  r.type = "button", r.onclick = () => ye(e), i.append(r, n("p", `${e.author} · ${e.genre}`, "book-meta"), n("p", e.rating_count ? `★ ${Number(e.rating_average).toFixed(1)} · ${e.rating_count} okur puanı` : "Henüz puanlanmadı", "community-summary"));
  const s = xe(e), o = n("button", "Ayrıntılar", "detail-button"), c = n("button", "☆ Puanla ve yorumla", "community-button"), p = n("button", l.compareIds.has(e.id) ? "✓ Seçildi" : "⇄ Karşılaştır", "compare-button");
  return o.type = "button", o.onclick = () => ye(e), c.type = "button", c.onclick = () => bt(e), p.type = "button", p.onclick = () => {
    l.compareIds.has(e.id) ? l.compareIds.delete(e.id) : l.compareIds.size < 4 ? l.compareIds.add(e.id) : f("En fazla dört kitap karşılaştırılabilir.", !0), z(), Ye();
  }, s.append(o, c, p), a.append(F(e), i, n("p", e.description, "summary"), ht(e), s), a;
}
function Ye() {
...
t("#compare-clear").onclick = () => {
  l.compareIds.clear(), t("#comparison-results").classList.add("hidden"), z(), Ye();
};
...
async function z(e = !1) {
  ...
  try {
    ...
    l.catalog.total = o.total, l.catalog.items = o.items, l.catalog.coverage = c, a.replaceChildren(...o.items.length ? o.items.map(ft) : [yt(s)]), t("#catalog-coverage").textContent = `${o.total} kitap · ${c.priced_books} kitapta ${c.offers} doğrulanmış teklif`, i && !s && (i.textContent = o.total.toLocaleString("tr-TR"), i.setAttribute("aria-busy", "false"));
...
async function bt(e) {
  t("#community-title").textContent = `${e.title} — ${e.author}`, t("#community-book-id").value = e.id, t("#community-dialog").showModal(), await Y(e.id);
}
...
t("#community-comment-form").onsubmit = async (e) => {
  ...
  r.disabled = !0;
  try {
    await g("/me/book-comments", { method: "POST", body: JSON.stringify({ book_id: a, parent_comment_id: i, content: t("#community-comment").value, contains_spoiler: t("#community-spoiler").checked }) }), e.target.reset(), t("#community-book-id").value = a, t("#community-parent-id").value = "", t("#community-reply-context").classList.add("hidden"), await Y(a), await be(), f(i ? "Yanıtın yayınlandı." : "Yorumun yayınlandı.");
  } catch (s) {
    f(s.message, !0);
  } finally {
    r.disabled = !1;
...
async function Y(e) {
  ...
  t("#community-rating").replaceChildren(...[1, 2, 3, 4, 5].map((u) => {
    ...
    return d.type = "button", d.title = `${u} yıldız`, d.setAttribute("aria-label", `${u} yıldız ver`), d.onclick = async () => {
      await g(`/me/book-ratings/${e}`, { method: "PUT", body: JSON.stringify({ rating: u }) }), await Y(e), await be(), await z(), f("Puanın kaydedildi.");
    }, d;
  }));
  ...
  [5, 4, 3, 2, 1].forEach((u) => {
    ...
    u.contains_spoiler && (h.tabIndex = 0, h.onclick = () => h.textContent = u.content, h.onkeydown = (x) => {
      x.key === "Enter" && h.click();
    });
    ...
    if (v.onclick = () => {
      ...
      await g(`/me/comments/${u.id}/helpful`, { method: u.own_helpful ? "DELETE" : "PUT" }), await Y(e);
    }, b.append(v, w), u.is_mine) {
      const x = n("button", "Yorumu sil", "text-button");
      x.onclick = async () => {
        await g(`/me/book-comments/${u.id}`, { method: "DELETE" }), await Y(e), await be();
      }, b.append(x);
    } else {
      const x = n("button", u.following_author ? "Takibi bırak" : "Takip et", "text-button"), S = n("button", "Şikâyet", "text-button");
      x.onclick = async () => {
        await g(`/me/follows/${u.author.id}`, { method: u.following_author ? "DELETE" : "PUT" }), await Y(e);
      }, S.onclick = () => {
...
async function ye(e) {
  ...
  try {
    ...
    if (m.append(n("h3", "Fiyat geçmişi")), y.length) {
      ...
      y.forEach((w) => {
        const x = n("i");
        x.style.height = `${20 + (v - w.price_minor) / Math.max(1, v - b) * 70}%`, x.title = `${N(w.price_minor, w.currency)} · ${new Date(w.observed_at).toLocaleDateString("tr-TR")}`, h.append(x);
      }), m.append(h);
...
function vt(e, a) {
  ...
  return r.width = 760, r.height = 230, r.setAttribute("aria-label", "Gerçek ve tahmini fiyat grafiği"), r.setAttribute("role", "img"), s.append(n("span", "Gerçek fiyat", "actual"), n("span", "15 günlük tahmin", "forecast")), i.append(r, s), requestAnimationFrame(() => {
    ...
    if (!d.length) return;
    const m = { x: 44, y: 24, b: 34 }, y = r.width - m.x - 18, h = r.height - m.y - m.b, b = Math.min(...d) * 0.96, v = Math.max(...d) * 1.04, w = Math.max(1, v - b), x = (_) => m.x + _ / Math.max(1, u.length - 1) * y, S = (_) => m.y + (v - _) / w * h;
    ...
    p.length && (o.fillStyle = "rgba(213,154,69,.14)", o.beginPath(), p.forEach((_, T) => o.lineTo(x(c.length + T), S(_.high))), [...p].reverse().forEach((_, T) => o.lineTo(x(u.length - 1 - T), S(_.low))), o.closePath(), o.fill());
    const A = (_, T, L, B = !1) => {
      o.beginPath(), o.strokeStyle = L, o.lineWidth = 3, o.setLineDash(B ? [7, 6] : []), _.forEach((te, k) => {
        const $ = x(T + k), C = S(te.value);
        k ? o.lineTo($, C) : o.moveTo($, C);
...
ye = async function(e) {
  ...
  try {
    ...
    if (y.append(n("p", "FİYAT ZEKÂSI", "eyebrow"), n("h3", "Geçmiş ve 15 günlük görünüm")), h.status === "ready") {
      ...
      b.append(n("div", `${N(h.current_price_minor)}|Güncel en düşük`, "price-signal"), n("div", `${N(v.lowest_price_minor)}|30 günün en düşüğü`, "price-signal"), n("div", `${h.deal_score}/100|${h.deal_label}`, "price-signal")), [...b.children].forEach((x) => {
        const [S, A] = x.textContent.split("|");
        x.replaceChildren(n("strong", S), n("small", A));
      }), y.append(b, vt(o.price_history || [], o.price_forecasts || []));
...
M(".tab").forEach((e) => e.onclick = () => {
  l.shelf = e.dataset.shelf, M(".tab").forEach((a) => a.classList.toggle("active", a === e)), oe();
});
function Ue() {
...
function _t(e) {
  ...
  return r.type = "button", r.onclick = () => wt(e), i.append(r), a.append(F(e), i), a;
}
async function wt(e) {
  try {
...
function oe() {
  const e = Ue();
  if (e.length) {
    t("#library-grid").replaceChildren(...e.map(_t)), I();
    return;
  }
  ...
  c.type = p.type = "button", c.onclick = () => G("catalog"), p.onclick = () => ce(), o.append(c, p), a.append(i, r, s, o), t("#library-grid").replaceChildren(a), I();
}
function xt(e) {
  const a = le(e) || e;
...
t("#progress-form").onsubmit = async (e) => {
  e.preventDefault();
  const a = t("#progress-book-id").value, i = t("#progress-book-id").dataset.custom === "1", r = i ? [...l.profile.read_books, ...l.profile.reading_books, ...l.profile.to_read_books].find((o) => o.id === a) : l.books.find((o) => o.id === a), s = le(r) || r;
  try {
    ...
    await g(i ? `/me/custom-books/${encodeURIComponent(a)}` : "/me/library", { method: "PUT", body: JSON.stringify(i ? Ge(r, o) : { book_id: a, ...o }) }), t("#progress-dialog").close(), await D(), f("Okuma ilerlemesi güncellendi.");
  } catch (o) {
    f(o.message, !0);
  }
};
...
async function D() {
  [l.profile, l.dashboard, l.gamification, l.alerts, l.notifications] = await Promise.all([g("/me/profile"), g(`/me/reading-dashboard?year=${(/* @__PURE__ */ new Date()).getFullYear()}`), g("/me/gamification"), g("/me/price-alerts"), g("/me/notifications")]), X(), oe(), Fe(), ke();
}
async function be() {
  l.gamification = await g("/me/gamification"), X(), de();
}
function Fe() {
  ...
  })), de(), Tt(e), Lt(e.genre_distribution), Et(e.series_progress), St();
}
function ve(e) {
  return e.xp_reward >= 100 ? "gold" : e.xp_reward >= 60 ? "silver" : "bronze";
}
function re(e, a = !1) {
  const i = n("span", "", `badge-medallion ${ve(e)}${a ? " large" : ""}`), r = n("span", e.icon, "badge-symbol");
...
function de() {
  ...
  t("#badge-showcase").replaceChildren(...o.length ? o.map((d) => {
    const m = n("div", "", `showcase-badge ${ve(d)}`);
    return m.append(re(d, !0), n("strong", d.name), n("small", `+${d.xp_reward} XP`)), m;
  ...
  if (u.replaceChildren(), p) {
    u.append(re(p), n("div", "", "motivation-copy"));
  ...
  t("#badge-grid").replaceChildren(...e.badges.map((d) => {
    const m = ve(d), y = n("article", "", `badge-card ${m}${d.earned ? " earned" : " locked"}${d.showcased ? " showcased" : ""}`), h = n("div", "", "badge-visual"), b = n("span", d.earned ? "KAZANILDI" : `%${d.progress_percent}`, `badge-status${d.earned ? " won" : ""}`);
    h.append(re(d, !0), b);
    const v = n("h4", d.name), w = n("span", `+${d.xp_reward} XP`, "badge-reward");
    y.append(h, v, w, n("p", d.description, "muted"));
    const x = n("div", "", "badge-progress"), S = n("div", "", "progress-bar"), A = n("i");
    ...
    if (x.append(S, n("small", T, "muted")), y.append(x), d.earned) {
      const L = n("button", d.showcased ? "✓ Vitrinde" : "Vitrinde göster", d.showcased ? "ghost badge-toggle active" : "ghost badge-toggle");
      L.type = "button", L.setAttribute("aria-pressed", String(d.showcased)), L.onclick = () => Ct(d.code, L), y.append(L);
    }
    return y;
  }));
}
async function Ct(e, a) {
  ...
  a.disabled = !0;
  try {
    l.gamification = await g("/me/gamification/showcase", { method: "PUT", body: JSON.stringify({ badge_codes: i }) }), X(), de(), f(r >= 0 ? "Rozet vitrinden kaldırıldı." : "Rozet profiline eklendi.");
  } catch (s) {
...
function ke() {
  ...
  const a = l.notifications.length ? l.notifications.map((i) => {
    ...
    if (s.append(n("strong", i.title), n("p", i.body, "muted"), n("small", new Date(i.created_at).toLocaleString("tr-TR"), "muted")), i.read_at)
      ...
      o.onclick = () => zt(i.id), r.append(s, o);
    }
    return r;
    ...
    try {
      ...
      e.classList.remove("hidden"), e.onclick = async () => {
        ...
        try {
          ...
          const s = await i.pushManager.subscribe({ userVisibleOnly: !0, applicationServerKey: Mt(a.vapid_public_key) }), o = s.toJSON();
...
async function je() {
  ...
  try {
    ...
    e.replaceChildren(...s.map((p) => Je(...p))), Dt(t("#catalog-health-chart"), Ot(r, i), "Kapak bütünlüğü", `${r} / ${i} kitapta kapak mevcut`), Nt(t("#catalog-risk-chart"), [{ label: "Eksik kapak", value: a.missing_covers, tone: "amber" }, { label: "Tekrar eser", value: a.duplicate_works, tone: "blue" }, { label: "Şüpheli kayıt", value: a.suspicious_records, tone: "red" }]);
  ...
  try {
    ...
    t("#catalog-issues").replaceChildren(...e.length ? e.map((i) => {
      ...
      return s.append(n("strong", i.title || i.book_id), n("p", `${i.issue_type} · ${i.severity}`, "muted")), [["Çözüldü", "resolved"], ["Yok say", "dismissed"]].forEach(([c, p]) => {
        ...
        u.onclick = async () => {
          await g(`/admin/catalog/issues/${i.id}`, { method: "PATCH", body: JSON.stringify({ status: p }) }), await $e();
        }, o.append(u);
      }), r.append(s, o), r;
...
t("#admin-nav").addEventListener("click", $e);
t("#admin-nav").addEventListener("click", () => ee().catch((e) => f(e.message, !0)));
t("#admin-nav").addEventListener("click", () => Ve().catch((e) => f(e.message, !0)));
async function Ve() {
...
async function ee() {
  ...
  const e = t("#admin-user-search").value.trim(), a = e ? `?q=${encodeURIComponent(e)}` : "", [i, r, s, o, c] = await Promise.all([g("/admin/dashboard"), g("/admin/logs?limit=30"), g(`/admin/users${a}`), g("/admin/community/reports?status=open"), g("/admin/metrics")]), p = ((te = c.routes) == null ? void 0 : te["/auth/login"]) || {}, u = c.business || {}, d = [[i.users, "Toplam kullanıcı", "♙", "amber"], [i.verified_users, "Doğrulanmış", "◇", "green"], [i.banned_users, "Banlı hesap", "♙", "green"], [o.length, "Açık şikâyet", "⚑", "amber"]];
  t("#admin-social-metrics").replaceChildren(...d.map((k) => Je(...k)));
  ...
  t("#admin-community-chart").replaceChildren(...m.map((k) => {
    const $ = n("div", "", "chart-column"), C = n("div", "", "chart-bar-slot"), E = n("i", "", `tone-${k.tone}`), O = Number(k.value || 0);
    return E.style.setProperty("--height", O ? `${Math.max(7, O / y * 100)}%` : "0%"), C.append(E), $.append(n("b", k.value), C, n("span", k.label)), $;
  }));
  const h = Math.round(Number(u.login_failure_rate || 0) * 100), b = Math.max(0, 100 - h), v = Number(p.p95_ms || 0), w = Number(u.suspicious_login_attempts || 0), x = c.alerts || [];
  t("#admin-alert-count").textContent = `${x.length} uyarı`, t("#admin-alert-count").className = `status-chip ${x.length ? "status-warn" : "status-good"}`, t("#admin-ops-health").replaceChildren(...[[`${b}%`, "Giriş başarısı", b, "green"], [`${v} ms`, "Giriş P95", Math.min(100, v / 1500 * 100), v >= 1500 ? "red" : "blue"], [w, "Şüpheli giriş", Math.min(100, w / 10 * 100), w >= 10 ? "red" : "amber"]].map(([k, $, C, E]) => {
    const O = n("div", "", "ops-item"), R = n("div", "", "ops-track"), ae = n("i", "", `tone-${E}`);
    return ae.style.width = `${C}%`, R.append(ae), O.append(n("strong", k), n("span", $), R), O;
  }));
  const S = i.top_books || [], A = Math.max(1, ...S.map((k) => Number(k.rating_count || 0)));
  t("#admin-top-books").replaceChildren(...S.length ? S.slice(0, 6).map((k, $) => {
    const C = n("div", "", "rank-row"), E = n("div"), O = n("div", "", "rank-track"), R = n("i");
    return E.append(n("span", `${$ + 1}`, "rank-number"), n("b", k.title), n("small", `★ ${Number(k.rating_average || 0).toFixed(1)} · ${k.rating_count}`)), R.style.width = `${Math.max(4, Number(k.rating_count || 0) / A * 100)}%`, O.append(R), C.append(E, O), C;
  }) : [n("p", "Henüz topluluk puanı yok.", "muted")]);
  const _ = { success: 0, warning: 0, error: 0 };
  r.forEach((k) => {
    const $ = Number(k.status_code || 0);
    k.level === "error" || $ >= 500 ? _.error++ : k.level === "warning" || $ >= 400 ? _.warning++ : _.success++;
  });
  const T = Math.max(1, r.length), L = n("div", "", "log-stack");
  Object.entries(_).forEach(([k, $]) => {
    const C = n("i", "", k);
    C.style.width = `${$ / T * 100}%`, L.append(C);
  });
  const B = n("div", "", "log-legend");
  [["success", "Başarılı"], ["warning", "Uyarı"], ["error", "Hata"]].forEach(([k, $]) => B.append(n("span", `${$} ${_[k]}`, k))), t("#admin-log-chart").replaceChildren(L, B), t("#system-logs").replaceChildren(...r.length ? r.slice(0, 6).map((k) => {
    const $ = n("div", "", `admin-row log-${k.level}`);
    return $.append(n("strong", `${k.status_code || "—"} ${k.route || k.event_type}`), n("small", `${k.duration_ms || 0} ms`)), $;
  }) : [n("p", "Henüz sistem kaydı yok.", "muted")]), t("#admin-users").replaceChildren(...s.length ? s.map((k) => {
    const $ = n("div", "", "admin-row"), C = n("div"), E = n("button", "Yönet", "ghost");
    return C.append(n("strong", `${k.display_name}${k.is_verified ? " ✓" : ""}`), n("p", `${k.app_role || "user"} · ${k.is_banned ? "BANLI" : k.rating_count || "0 puan, " + (k.comment_count || 0) + " yorum"}`, "muted")), E.onclick = () => Rt(k), $.append(C, E), $;
    ...
    var R;
    const $ = n("div", "", "admin-row"), C = n("div"), E = n("div", "", "actions"), O = k.content || ((R = k.comment) == null ? void 0 : R.content) || "";
    return C.append(n("strong", k.reason), n("p", O, "muted")), [["İncele", "reviewing", null], ["Kaldır", "resolved", "removed"], ["Reddet", "dismissed", null]].forEach(([ae, Xe, Ze]) => {
      const ne = n("button", ae, "ghost");
      ne.onclick = async () => {
        ne.disabled = !0;
        try {
          await g(`/admin/community/reports/${k.id}`, { method: "PATCH", body: JSON.stringify({ status: Xe, comment_status: Ze }) }), await ee();
        } finally {
          ne.disabled = !1;
        }
      }, E.append(ne);
...
async function Ce() {
  ...
  t("#pipeline-summary").replaceChildren(...i ? [["Durum", i.status], ["Kontrol", i.checked_count], ["Başarılı", i.success_count], ["Hata", i.failure_count]].map(([r, s]) => {
    ...
    return s.type = "button", s.append(n("strong", `${r.job_type} · ${r.status}`), n("small", `${new Date(r.started_at).toLocaleString("tr-TR")} · ${r.success_count}/${r.checked_count}`)), s.onclick = async () => De(await g(`/admin/pipelines/logs?run_id=${encodeURIComponent(r.id)}&limit=100`)), s;
  }) : [n("p", "Henüz çalışma yok.", "muted")]), De(a);
}
function De(e) {
...
t("#admin-nav").addEventListener("click", () => Ce().catch((e) => f(e.message, !0)));
t("#admin-refresh").onclick = async () => {
  const e = t("#admin-refresh");
  e.disabled = !0, e.innerHTML = '<span aria-hidden="true">↻</span> Yenileniyor…';
  try {
    await Promise.all([je(), ee(), Ve(), Ce()]), f("Yönetim verileri güncellendi.");
  } catch (a) {
    f(a.message, !0);
  } finally {
...
t("#run-price-pipeline").onclick = async () => {
  ...
  try {
    ...
    await g("/admin/pipelines/prices", { method: "POST", body: JSON.stringify({ idempotency_key: a, limit: 20, discover_books: 5, retailers: ["kitapsec", "kitapsepeti", "bkmkitap"], refresh_existing: !0 }) }), f("Fiyat hattı arka planda başlatıldı."), setTimeout(() => Ce().catch((i) => f(i.message, !0)), 1200);
  } catch (a) {
    f(a.message, !0);
  } finally {
...
t("#admin-invitation-form").onsubmit = async (e) => {
  e.preventDefault();
  const a = e.currentTarget, i = t("#admin-invitation-submit"), r = t("#admin-invitation-status"), s = { display_name: t("#admin-invitation-name").value.trim(), email: t("#admin-invitation-email").value.trim() };
  i.disabled = !0, i.textContent = "Gönderiliyor…", r.textContent = "";
  try {
    const o = await g("/admin/invitations", { method: "POST", body: JSON.stringify(s) });
    r.textContent = o.message, a.reset();
  } catch (o) {
    r.textContent = o.message;
  } finally {
    i.disabled = !1, i.textContent = "Davet gönder";
  }
};
let P = null;
function Rt(e) {
...
t("#admin-user-form").onsubmit = async (e) => {
  ...
  try {
    d && await g(`/admin/users/${r}/verification`, { method: "PATCH", body: JSON.stringify({ verified: s, label: s && o ? o : null }) }), m && await g(`/admin/users/${r}/ban`, { method: "PATCH", body: JSON.stringify({ banned: c, reason: c ? p : null, duration_days: c && u ? Number(u) : null }) }), t("#admin-user-dialog").close(), await ee(), f("Kullanıcı ayarları güncellendi.");
  } catch (y) {
...
t("#admin-user-search").oninput = () => {
  clearTimeout(Ne), Ne = setTimeout(() => ee().catch((e) => f(e.message, !0)), 250);
};
...
t("#queue-quality-scan").onclick = async () => {
  try {
    await g("/admin/catalog/jobs", { method: "POST", body: JSON.stringify({ job_type: "quality_scan", limit: 200 }) }), await $e(), f("Kalite taraması kuyruğa eklendi.");
  } catch (e) {
    f(e.message, !0);
  }
};
let ze = !1;
function q(e, a, i = [], r = !0) {
...
function U(e) {
  t("#chat-suggestions").replaceChildren(...e.map((a) => {
    const i = n("button", a);
    return i.type = "button", i.onclick = () => Qe(a), i;
  }));
}
async function Se() {
  try {
...
function Te() {
  l.chatSessionId = null, l.chatHistory = [], t("#chat-messages").replaceChildren(), q("assistant", "Yeni bir konuşma başladı. Nasıl bir kitap aradığını anlatabilirsin.", [], !1), U(["Kısa ve atmosferik bir roman", "Şu an ne okuyorum?"]), t("#chat-session-select").value = "", t("#chat-delete").disabled = !0;
}
async function It(e) {
  if (!e) {
    Te();
  ...
  try {
    ...
    l.chatSessionId = e, l.chatHistory = [], t("#chat-messages").replaceChildren(), a.forEach((i) => q(i.role, i.content, i.books || [], !0)), t("#chat-delete").disabled = !1, U(["Konuşmaya devam et", "Benzer kitaplar bul"]);
  } catch (a) {
    f(a.message, !0);
  }
}
t("#chat-session-select").onchange = (e) => It(e.target.value);
t("#chat-new").onclick = Te;
t("#chat-delete").onclick = async () => {
  if (!(!l.chatSessionId || !confirm("Bu konuşma ve mesajları silinsin mi?")))
    try {
      await g(`/me/chat/sessions/${encodeURIComponent(l.chatSessionId)}`, { method: "DELETE" }), Te(), await Se(), f("Konuşma silindi.");
    } catch (e) {
      f(e.message, !0);
    }
};
function Kt() {
  ze || (ze = !0, q("assistant", "Merhaba! Kitaplar, yazarlar ve edebî kavramlar hakkında konuşabilir; ekrandaki sonuçları yorumlayabilir ve okuma yolculuğuna eşlik edebilirim.", [], !1), U(["Ekrandaki kitapları karşılaştır", "Roman ile novella farkı nedir?", "Yıllık hedefim nasıl gidiyor?"]), Se(), I());
}
...
function We() {
  const e = Bt();
  let a = [], i = null;
  return e === "discover" && !t("#recommendations-panel").classList.contains("hidden") ? (a = l.lastRecommendations, i = l.lastDiscoveryQuery) : e === "catalog" ? (a = l.catalog.items, i = t("#catalog-search").value.trim() || null) : e === "library" && (a = Ue()), { view: e, query: i, books: a.slice(0, 10).map((r, s) => Ht(r, s + 1)) };
}
function Le(e, a = !1) {
...
function I() {
  ...
  const e = We(), a = { discover: "eşleştirme", catalog: "katalog", library: "kitaplık", insights: "okuma analizi", alerts: "fiyat alarmı", quality: "kalite" };
  Le(e.books.length ? `${e.books.length} ${a[e.view]} kitabı bu sohbete bağlı.` : `${a[e.view]} ekranını görebiliyorum.`);
}
let me = !1;
async function Qe(e) {
  const a = e.trim();
  if (a.length < 2 || me) return;
  me = !0;
  ...
  try {
    ...
    l.chatSessionId = d.session_id, u.remove(), q("assistant", d.answer || "Yanıt oluşturulamadı.", d.books || []), U(d.suggestions || []), d.pending_action && await window.BookPusulasiUI.confirmAction(d.pending_action) && (await D(), q("assistant", "İşlem onayından sonra güvenle tamamlandı.", [], !1), f("Asistan işlemi tamamladı."));
  } catch (d) {
    q("assistant", d.message || "Asistan yanıtı tamamlanamadı."), U(["Tekrar dene", "Şu an ne okuyorum?"]);
  } finally {
    clearTimeout(p), u.remove(), me = !1, s.disabled = !1, o.disabled = !1, I(), s.focus();
  }
}
t("#chat-form").onsubmit = (e) => {
  e.preventDefault(), Qe(t("#chat-input").value).finally(Se);
};
let Re = !1;
function qt() {
  if (Re) return;
  ...
  function d(y, h, b, v, w, x = []) {
    i.save(), i.strokeStyle = `rgba(8,116,90,${w})`, i.lineWidth = 1, i.setLineDash(x), i.beginPath(), i.ellipse(y, h, b, v, -0.13, 0, Math.PI * 2), i.stroke(), i.restore();
  }
  function m(y) {
    i.clearRect(0, 0, o, c);
    const h = o / 2, b = c / 2;
    d(h, b, o * 0.31, c * 0.25, 0.17, [4, 7]), d(h, b, o * 0.41, c * 0.34, 0.09, [2, 10]), i.save(), i.fillStyle = "rgba(8,116,90,.16)", [[0.12, 0.2], [0.87, 0.3], [0.17, 0.78], [0.82, 0.82]].forEach(([v, w]) => {
      ...
      const w = v.angle + (r ? 0 : y * v.speed), x = v.orbit === 1, S = o * (x ? 0.39 : 0.3), A = c * (x ? 0.31 : 0.23), _ = (Math.sin(w) + 1) / 2, T = h + Math.cos(w) * S, L = b + Math.sin(w) * A, B = 0.76 + _ * 0.28;
...
document.addEventListener("click", (e) => {
  e.target.closest(".shelf-picker") || M(".shelf-picker[open]").forEach((a) => a.open = !1);
});
document.addEventListener("keydown", (e) => {
  e.key === "Escape" && (M(".shelf-picker[open]").forEach((a) => a.open = !1), t("#chat-panel").classList.contains("hidden") || t("#chat-close").click());
});
